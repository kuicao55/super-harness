...
Invoke the `claude-codex-harness:harness-entry` skill with context: this is a `/harness:execute` invocation.
...
Then immediately read and follow the `claude-codex-harness:harness-entry` skill, passing the context that this is a `/harness:execute` invocation.
