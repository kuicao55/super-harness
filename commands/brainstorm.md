...
Invoke the `harness:harness-entry` skill with context: this is a `/harness:brainstorm` invocation.
...
Then immediately read and follow the `harness:harness-entry` skill, passing the context that this is a `/harness:brainstorm` invocation.
